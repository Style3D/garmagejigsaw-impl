...
class AllPieceMatchingDataset_stylexd(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.use_stitch_noise:
            ...
            stitch_noise_strength = 0.05 * rand_param + 0.1 * (1 - rand_param)
        ...
        if self.mode == "train":
            ...
            if self.use_stitch_noise:
                ...
                # stitch_noise_strength=2 在董远数据上inference结果更好
                rand_param = random.random()
                stitch_noise_strength = 0.8 * rand_param + 2.2 * (1-rand_param)

                # === 对缝合点 按缝合线加远离噪声 ===
                stitch_noise_strength1 = stitch_noise_strength
                ...
                # === 对不缝合的点加随机噪声 ===
                stitch_noise_strength3 = stitch_noise_strength * (random.random()*0.4+0.2)
                ...
                # === 对缝合的点的两端加相同噪声（只加一点点） ===
                stitch_noise_strength4 = stitch_noise_strength * (random.random()*0.2+0.1)
