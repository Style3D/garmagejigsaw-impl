...
def pointstitch_2_edgestitch(batch, inf_rst, stitch_mat, stitch_indices,
                             ...
                             filter_para_len=0.15, expend_thresh=0.15):
    ...
    # [todo]为每个stitch_points_list从首位分别挑选一批分别属于同一个edge的点按照param进行排序
    ...
    for s_idx, stitch_points_list in enumerate(all_stitch_points_list):
        ...
        for point_idx, stitch_point in enumerate(stitch_points_list):
            ...
            if stitch_point[1]["edge_id"] != start_edge_id or point_idx == len(stitch_points_list)-1:
                if point_idx == len(stitch_points_list)-1:
    ...
    # [todo:OPT] 将一些被分隔开（globalparam出现了空隙）或是重叠的（globalparam出现了重合）的边缝合 合并到一起去
    pass
