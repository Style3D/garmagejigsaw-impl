...
def pointstitch_2_edgestitch(batch, inf_rst, stitch_mat, stitch_indices):
                ...
                for _ in range(1):
                    ...
                    if len(stitch_points_list) > 0:
                        ...
                        for p in point_info_cor:
                            if p["edge_id"] in [e["edge_id"] for e in stitch_points_list[-1][1]]:
                                trigger = True
                    ...
                    stitch_points_list.append([point_info, point_info_cor])
                    break
    # [todo] 如果stitch_points_list不为空，append到all
    if len(stitch_points_list) > 0:
        all_stitch_points_list.append(stitch_points_list)
    ...
    # 长为1的先删了试试看
    def filter_too_short(all_stitch_points_list, fliter_len = 3):
        all_stitch_points_list_ = []
        for s in all_stitch_points_list:
            if len(s)>fliter_len:
                all_stitch_points_list_.append(s)
        return all_stitch_points_list_
    all_stitch_points_list = filter_too_short(all_stitch_points_list, fliter_len = 1)
    ...
    for s_idx, stitch_points_list in enumerate(all_stitch_points_list):
        for sp_idx, st_point in enumerate(stitch_points_list):
            ...
            if sp_idx==0: compare_idx = 1
            else: compare_idx = sp_idx-1
    ...
    for s_idx, stitch_points_list in enumerate(all_stitch_points_list):
        ...
        for sp_idx, st_point in enumerate(stitch_points_list):
            ...
            # if st_point[0]["edge_id"] == stitch_points_list[sp_idx-1][0]["edge_id"]:
            ord = st_point[0]["param"]-stitch_points_list[sp_idx-1][0]["param"]
        ...
        for sp_idx, st_point in enumerate(stitch_points_list):
            if sp_idx == 0: continue
            # if st_point[1]["edge_id"] == stitch_points_list[sp_idx - 1][1]["edge_id"]:
            ord = st_point[1]["param"] - stitch_points_list[sp_idx - 1][1]["param"]
            ord = 1 if ord > 0 else -1
            order_list.append(ord)
        order_sum = sum(order_list)
        ...
        # [TODO] 对于错误情况的简单解决方法，需要排查原因
        if (stitch_edge[0]["start"]["param"] == stitch_edge[0]["end"]["param"] or
            stitch_edge[1]["start"]["param"] == stitch_edge[1]["end"]["param"]):
            continue
