...
class JointSegmentationAlignmentModel(MatchingBaseModel):
    ...
    def forward(self, data_dict):
        ...
        # [仅局部特征]
        pcs_feats  = self._extract_part_feats(pcs, batch_length)
        ...
        for name, layer in self.tf_layers:
            if name == "self":
                ...
                pcs_feats = (
                    layer(
                        pcs_flatten,
                        pcs_feats.view(-1, self.pc_feat_dim),
                        batch_length,
                    )
                    .view(B_size, N_point, -1)
                    .contiguous()
        ...
        # [仅局部特征]
        stitch_pcs_feats = self._get_stitch_pcs_feats(pcs_feats, n_stitch_pcs_sum, pc_cls_mask, B_size, N_point, self.pc_feat_dim)
